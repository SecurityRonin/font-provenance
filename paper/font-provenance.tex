...
% Build note (2026-07-24): the CJK build was verified with these macOS system
% fonts --- Songti TC (main; Chinese), Hiragino Mincho ProN (Japanese, via \ja),
% Apple SD Gothic Neo (Korean, via \ko).
% =============================================================================
...
% --- Script helpers ----------------------------------------------------------
% Build note (2026-07-24): build testing showed Songti TC (main CJK) carries the
% Simplified and Traditional Chinese glyphs used here but not Japanese kana or
% Korean hangul, so the per-script macros switch fonts for those scripts.
...
facts justify this choice. First, the Unicode consolidation era decoupled CJK font
availability from retail regional editions: Windows 2000 carried the East Asian fonts
on every edition's installation media (installable via Regional Options---though
pre-Vista Windows still shipped single-language OS images, with MUI an add-on), and
Mac OS X and the mobile OSes bundled multi-script font sets from their first releases;
what varies by locale is primarily the per-locale default UI font binding and, in
office software, the per-language document defaults---not whether the CJK fonts can be
present.\cite{m-w2k,m-w2k-2} Second, regional editions matter as distinct artifacts
...
Museum.\cite{w-31date,w-ttf,w-mssans}
...
\fpevent{2000}{Windows 2000 (Feb 17)---East Asian fonts carried on every edition's install media (installable via Regional Options, not default-installed)---SimSun (SC), PMingLiU (TC), MS UI Gothic (JP), Gulim (KR) as locale UI fonts}
...
\fpevent{2008}{Android 1.0 (SDK Sep 23, first handset Oct)---Droid Sans default UI, Droid Serif and Droid Sans Mono bundled, Droid Sans Fallback for CJK}
...
\item \textbf{2008-09-23 --- Android 1.0 (SDK release 1; the first shipping handset, the
T-Mobile G1, reached customers in October 2008).} \textbf{Droid Sans} is the default UI font;
...
release---the same typeface design under two brands (the released fonts differ in
naming tables and packaging)---with partner foundries SinoType (SC), Iwata
...
\fpevent{1999}{Office 2000 (retail Jun 10)---Word default becomes Times New Roman 12\,pt}
...
New Roman 10\,pt}; Excel \textbf{Arial 10}. \textbf{Office 2000 (launch announcement
1999-06-07; retail availability 1999-06-10)} raised
...
\item \textbf{Traditional Chinese:} \textbf{PMingLiU 新細明體 --- no change found.} In
the sources searched (a scoped negative finding on secondary records,
\S\ref{sec:method}), the TC edition retained PMingLiU as its default through current
Microsoft 365; Microsoft JhengHei
...
versions are reported to default to Hancom's own \textbf{\ko{함초롬바탕} HCR Batang}
[secondary --- UNCERTAIN] (Hamchorom family;
...
32000-1 \S9.6.2.2). Acrobat on Windows maps them to metrically compatible system
faces (Helvetica$\rightarrow$Arial, Times$\rightarrow$Times New Roman,
Courier$\rightarrow$Courier New); other viewers ship or select their own metrically
...
inside an otherwise foreign document is consistent with post-hoc Acrobat editing---
corroborate via Acrobat metadata or incremental-update history, since Minion Pro is a
widely distributed Adobe typeface in its own right.\cite{p-tools,p-tools-2,p-minion,p-minion-2}
...
composition evolved across Acrobat 7$\rightarrow$DC; the per-version, per-lane pack
composition is reported from vendor and user documentation and is UNCERTAIN at
per-version level). \emph{Provenance:} unembedded CJK text rendering in these faces is
consistent with Acrobat substitution; the same families are used in ordinary
typesetting, so corroborate with embedding status and document metadata.\cite{p-cjk,p-cjk-2,p-cjk-3}
\end{itemize}
...
\addlinespace
Aspose.Cells & Calibri 11 (reported --- UNCERTAIN as cited, see note) & Falls back to workbook \code{DefaultStyle.Font}; PDF/image save options can override & Calibri mirrors Excel 2007+ \\
\addlinespace
...
\addlinespace
Aspose.PDF & Times New Roman (reported --- UNCERTAIN as cited, see note) & Exact-name match, then \code{FontRepository.Substitutions}; Base-14 never embedded & Times New Roman re-set on edited text \\
\bottomrule
\end{tabular}
\end{table*}

Key documented facts: Aspose.Words' \code{DefaultFont\allowbreak{}Substitution\allowbreak{}Rule.\allowbreak{}Default\allowbreak{}FontName}
...
Peers, for contrast---only openpyxl is source-verified here; the others are commonly
documented, version- and API-dependent, and UNCERTAIN as cited: \textbf{openpyxl}
hardcodes Calibri 11 (\code{DEFAULT\_FONT} in \code{styles/fonts.py}); \textbf{iText}'s
text APIs are documented as defaulting to Helvetica; \textbf{Apache PDFBox} has no
implicit default (the caller chooses); \textbf{python-docx} inherits Calibri 11 from
its bundled template; \textbf{GemBox} documentation reports Calibri defaults.\cite{as-peers}
